...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-2xl overflow-hidden border border-neutral-800 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#c4ff00]/5 flex flex-col h-full group">
      {/* Image Container */}
      <Link to={`/home/shop/${id}`} className="relative h-56 w-full bg-white overflow-hidden block">
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="mt-auto flex items-center justify-between">
          ...
          <button className="flex items-center gap-2 bg-[#c4ff00] hover:bg-[#b0e600] text-black px-4 py-2 rounded-xl font-semibold transition-colors text-sm">
            <FiShoppingCart /> Add
          </button>
        </div>
      </div>
